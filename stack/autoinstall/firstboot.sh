...
#   2. `docker compose up -d` — pulls images (tracker uses the locally-built
#      naglight:local tag), starts all services with restart:unless-stopped.
#   3. Wait for Technitium to be healthy, then run provision-technitium.sh
#      (zero-touch DNS: zone, split-horizon records, forwarders, blocklists).
#   4. Point the HOST resolver at the local Technitium so the box itself uses it.
#   5. Stamp .provisioned.
#
...
# ── 3. Technitium zero-touch provisioning ────────────────────────────────────
...
# ── 4. make the host itself use local DNS ────────────────────────────────────
