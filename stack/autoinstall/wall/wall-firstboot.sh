...
WALL_AUDIO_MIC_CAPTURE_PERCENT=${WALL_AUDIO_MIC_CAPTURE_PERCENT:-24}
...
EOF
    ...
    log "audio: mic knobs seeded - capture 24% with no boost (the level that stopped it clipping), rear pair at 66"
fi

# The Bluetooth mic return's own poll interval, in its own file because
# wall-bt-mic.service is not root-only and has no business reading wall.env.
