...
| `wall-occupancy.py` + `wall-occupancy.service` + `wall-occupancy.timer` | SN-015 occupancy power, **off unless `WALL_ABSENCE_ENABLED=true`**. A fixed one-minute cadence, so unlike the two above this `.timer` is TRACKED, not generated; firstboot step 6b enables or disables it. `wall-occupancy.py` is the pure decision and `wall-sleep.sh occupancy` applies it — ONE record carrying BOTH the backlight and the suspend/RTC action, so the two can never drift. Presence arrives as IF-012's `WALL_PRESENCE_FILE`, and anything short of a fresh positive "absent" reads as PRESENT. |
