...
def build_post(reading, last, goal_lb, now):
    ...
    if reading is not None:
        ...
        return gauge_body(value_lb, goal_lb, observed_at), True
    if last is not None:
        return gauge_body(last["value"], goal_lb, last["observed_at"]), False
...
def open_for_write(path, state_path):
    ...
    Contract:
      Inputs:  path: the file about to be written; state_path: the one file
               this feeder owns (WEIGHT_STATE_FILE).
      ...
      Raises:  PermissionError for ANY path that is not `state_path` or its
               ...
               anything whose basename is a known credential file.

    WHY AN ALLOW-LIST AND NOT A DENY-LIST - B7's shape, kept verbatim because
    the reasoning is not usage-specific. "Never writes a credential file"
    cannot be met by listing the files we know about; the next vendor tool puts
    its token somewhere new. So the feeder declares the ONE path it may write
    and refuses the rest of the filesystem, and the credential basenames are a
    SECOND check that exists only to make the failure message name the real
    hazard when someone points the state file at a token.

    THE GOOGLE HEALTH REFRESH TOKEN IS READ-ONLY TO THIS PROCESS and is not on
    the allow-list. A refresh token that this feeder could rewrite is a token
    that a bug could park somewhere with looser permissions, and a refresh
    token the Owner minted through a browser does not survive being truncated -
    it costs a person a trip to a browser to get back.
    ...
    if os.path.basename(resolved) in CREDENTIAL_BASENAMES:
        raise PermissionError(
            "REFUSED: %s is a credential file. This feeder reads credentials "
            "and never writes one." % resolved)
...
def load_state(state_path):
    """Last successful reading. A missing or corrupt file is {}."""
    try:
...
def post_gauge(body, url, env, timeout):
    """POST the gauge. Returns (ok, detail); never raises on an HTTP failure."""
...
def run_cycle(env, now=None, readers=None, poster=None, goal_loader=None):
    ...
    try:
        ...
        failures.append("%s: unexpected %s: %s" % (source, type(exc).__name__, exc))
